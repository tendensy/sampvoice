...
void Plugin::MainLoop()
{
    ...
    int defaultKey = pLoader->Get().iVoiceChatKey; // 66
    if(KeyFilter::IsKeyAvailable(defaultKey) && MicroIcon::IsShowed() && pLoader->Get().bVoiceChatEnable)
    {
        ...
        ImGui::Begin("VoiceButton", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings);
        
        ImVec2 vecButSize = ImVec2(pLoader->Get().fVoiceChatSize * 5 + 5.0f, pLoader->Get().fVoiceChatSize * 5);
        ...
        ImGui::SetWindowPos(ImVec2(pGUI->ScaleX(pLoader->Get().fVoiceChatPosX), pGUI->ScaleY(pLoader->Get().fVoiceChatPosY)));
        ImGui::End();
